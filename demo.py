...
if __name__ == '__main__':
    ...
    ret_masks, ret_scores = gen_masks_new(
        net, img_blob, config, dest_shape=(origin_height, origin_width))
    ...
    for i in np.argsort(ret_scores)[::-1][:20]:
        ...
        img_org = mark_boundaries(
            img_org, mask[:, :, 0], color=color.tolist(), mode='thick')
