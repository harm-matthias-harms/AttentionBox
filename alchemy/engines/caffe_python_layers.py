...
class AlchemyDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # queue of job (all images in dataset)
        q = multiprocessing.Queue(len(self.spider().dataset))
        self.q = q
        ...
        self.process = multiprocessing.Process(
            target=self.fetch, args=(self.spider(),))
        self.process.start()
    ...
    def fetch(self, spider):
        while self.q.qsize() > 0:
            try:
                ...
                # send pre-processed datat back to master
                self.q2.put([index, i, item], True)
            except Empty:
                pass
    ...
    def forward(self, bottom, top):
        self.iterations += 1
        try:
            printProgress(self.iterations, config.steps,
